...
UDPcommand_libcamera = "libcamera-vid --width {0} --height {1} --framerate {2} --bitrate {3} --quality {6} --timeout 0 --verbose --nopreview --raw - | gst-launch-1.0 -e -vvvv fdsrc ! h264parse ! rtph264pay config-interval=1 pt=96 ! udpsink host={4} port={5}"
TCPcommand_libcamera = "libcamera-vid --width {0} --height {1} --framerate {2} --bitrate {3} --quality {6} --timeout 0 --verbose --nopreview --raw - | gst-launch-1.0 -v fdsrc ! h264parse !  rtph264pay config-interval=1 pt=96 ! gdppay ! tcpserversink host={4} port={5}"
